...
from n3rv.config import DEFAULTS
from n3rv.providers.base import ModelProvider
from n3rv.providers.fallback import FallbackProvider
from n3rv.providers.tts import TTSProvider
from n3rv.providers.tts_fallback import TTSFallbackProvider

# Registry: name → fully-qualified class path
_PROVIDER_REGISTRY: dict[str, str] = {
    "qwen": "n3rv.providers.qwen.QwenProvider",
    "openai": "n3rv.providers.openai.OpenAIProvider",
    "local": "n3rv.providers.local.LocalProvider",
    "fallback": "n3rv.providers.fallback.FallbackProvider",
}
...
def get_provider(name: str | None = None) -> ModelProvider:
    ...
    If ``name`` is ``None``, the ``N3RV_PROVIDER`` env var is read,
    defaulting to ``"qwen"``.

    =====================  ================================================
    Input                  Result
    =====================  ================================================
    ``None``               Read ``N3RV_PROVIDER`` env, default ``"qwen"``
    ``"qwen"``             ``QwenProvider()`` with env defaults
    ``"qwen:model-id"``    ``QwenProvider(model="model-id")``
    ``"fallback"``         Parse ``N3RV_FALLBACK_PROVIDERS`` env
    ...
    =====================  ================================================
    """
    resolved_name = (name or os.environ.get("N3RV_PROVIDER") or DEFAULTS.provider).strip().lower()
    ...
    if provider_name == "fallback":
        if model_override is not None:
            raise ValueError(
                ...
                "Configure individual entries in N3RV_FALLBACK_PROVIDERS."
            )
...
def _build_fallback() -> FallbackProvider:
    """Build a FallbackProvider from N3RV_FALLBACK_PROVIDERS env var."""
    raw = os.environ.get("N3RV_FALLBACK_PROVIDERS")
    if not raw or not raw.strip():
        raise ValueError("N3RV_FALLBACK_PROVIDERS is not set. Set it to a comma-separated list of provider names.")
...
def get_tts_provider(
    *,
    ...
    ``N3RV_TTS_FALLBACK_MODELS`` environment variable is set, returns
    ...
    """
    fallback_models = os.environ.get("N3RV_TTS_FALLBACK_MODELS", "").strip()

    if fallback_models:
        model_ids = [m.strip() for m in fallback_models.split(",") if m.strip()]
        if not model_ids:
            raise ValueError(
                "N3RV_TTS_FALLBACK_MODELS is set but empty. Set it to a comma-separated list of TTS model IDs."
            )
        providers = [
            TTSProvider(
                model=mid,
