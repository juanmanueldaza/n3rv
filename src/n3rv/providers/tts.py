...
# Set via N3RV_TTS_BASE_URL env var or the base_url constructor arg.
#
...
class TTSProvider:
    ...
    base_url:
        API base URL. Falls back to ``N3RV_TTS_BASE_URL`` env var,
        then ``N3RV_BASE_URL`` env var,
        then ``https://dashscope-intl.aliyuncs.com/api/v1``.
    voice:
        Default voice. Falls back to ``N3RV_TTS_VOICE`` env var,
        then ``Cherry``.
    """
    ...
    def __init__(
        ...
        self.base_url = (
            base_url or os.environ.get("N3RV_TTS_BASE_URL") or os.environ.get("N3RV_BASE_URL") or _DEFAULT_BASE_URL
        )
        self.voice = voice or os.environ.get("N3RV_TTS_VOICE") or _DEFAULT_VOICE
