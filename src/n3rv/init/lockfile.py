...
The lockfile (``.n3rv/template-lock.json``) records the hash of every
file rendered by ``n3rv init`` or ``n3rv update``. On subsequent
...
logger = logging.getLogger("n3rv.lockfile")
...
def lockfile_path(root: Path) -> Path:
    ...
    return root / ".n3rv" / LOCKFILE_NAME
