...
def _get_audio_duration(path: Path) -> float:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not determine audio duration for {path}: {e}. "
            f"The file may be empty or corrupted."
        ) from e
    except subprocess.TimeoutExpired:
        raise RuntimeError(
            f"ffprobe timed out reading {path}. "
            f"The file may be corrupted or unusually large."
        ) from None
    except subprocess.CalledProcessError as e:
        raise RuntimeError(
            f"ffprobe failed for {path}: {e.stderr.strip() if e.stderr else e}"
        ) from e
