...
N3RV ships a GitHub Spec Kit–style workflow as an alternative, lighter-weight path built on
top of the same memory infrastructure. It focuses on three artifacts — a **spec**, a
**plan**, and a **task list** — driven by the `/spec-kit` command or the `sdd-spec-kit-*`
skills.

### Artifacts

| Artifact | Memory key | Purpose |
|----------|------------|---------|
| Spec | `sdd-<change_id>-spec-kit-spec` | Testable feature spec |
| Plan | `sdd-<change_id>-spec-kit-plan` | Implementation plan |
| Tasks | `sdd-<change_id>-spec-kit-tasks` | Ordered task checklist |

### On-Disk Templates

`n3rv init` writes the canonical GitHub Spec Kit "core pack" templates (byte-identical to
`specify init`) to the target project under `.specify/templates/`, so projects scaffolded by
n3rv are interoperable with spec-kit-driven repos. A `NOTICE.md` preserves the MIT attribution
for these upstream templates.

- `spec-template.md` — Feature spec: User Scenarios & Testing (prioritized, independently
  testable), Edge Cases, Requirements (FR-xxx), Key Entities, Success Criteria (SC-xxx), Assumptions
- `plan-template.md` — Implementation plan: Summary, Technical Context, Constitution Check,
  Project Structure, Complexity Tracking
- `tasks-template.md` — Task checklist: setup / foundational / user stories (per priority) / polish
- `constitution-template.md` — Project principles template (for spec-kit consumption)
- `checklist-template.md` — Reviewer-owned requirements-quality checklist

The `spec`, `plan`, and `tasks` scaffolds share a common contract: each user story is an
independently implementable and testable slice, the plan is traceable to the spec, and tasks are
sequenced so earlier ones unblock later ones.

### Principles

- Every acceptance criterion must be testable (binary pass/fail).
- User stories are prioritized (P1/P2/P3) and each is independently implementable/testable.
- The plan must be traceable to the spec's user stories and requirements.
- Tasks follow the canonical format `- [ ] [ID] [P?] [Story] Description` and are sequenced so
  earlier tasks unblock later ones; each user story is grouped so it is independently testable.
- The constitution (`constitution.md` generated by `init`) documents these principles and is a
  binding gate on every spec/plan.
